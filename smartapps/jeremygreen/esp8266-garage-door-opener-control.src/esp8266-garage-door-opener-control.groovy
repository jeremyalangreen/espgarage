/**
 *  ESP8266 Garage Door Opener Control
 *
 ...
 */
 definition(
    name: "ESP8266 Garage Door Opener Control",
    namespace: "jeremygreen",
    author: "Jeremy Green",
    description: "Control your garage door opener with an ESP8266",
 ...
 */
def getDeviceType() {
    return "urn:jeremygreen:device:espgarage:1"
}
 ...
 */
def getDeviceName() {
    return "ESP8266 Garage Door Opener Controller"
}
...
private def parseDESC(xmlData) {
    ...
    def devicetxml = xmlData.device.deviceType.text().toLowerCase()

    // Make sure it's the type we want
    if (devicetxml == devicetype) {
        log.info "parseDesc() xml devicetype matches"
...
private discoverDevices() {
    log.trace "discoverDevice() Issuing SSDP request for ${getDeviceType()}"
...
private verifyDevice(String deviceNetworkId, String ip, int port, String devicessdpPath) {
    log.trace "verifyDevice() ${ip}:${port} ${devicessdpPath} "
    if(ip) {
        def address = ip + ":" + port
